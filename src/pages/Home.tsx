        ...
        {products.slice(0, 3).map((prod, index) => (
          ...
          >
            <Link to={`/product/${prod.id}`}>
              <div className="mobile-product-card product-card glass rounded-xl shadow-md border border-white/20 group transition-all duration-300">
                <div className="relative overflow-hidden rounded-lg mb-2">
                  ...
                  <button
                    ...
                    className="absolute top-1.5 right-1.5 w-7 h-7 sm:w-8 sm:h-8 rounded-full bg-white/90 backdrop-blur-sm flex items-center justify-center shadow-md hover:scale-110 transition-all duration-200"
                  >
                    <FiHeart
                      size={14}
                ...
                <div className="product-info">
                  <div className="mobile-product-title text-[#111]">{prod.name[lang]}</div>
                  <div className="mobile-product-price">{prod.price} {t("egp")}</div>
                  <p className="text-gray-600 text-xs mt-1 line-clamp-2 hidden sm:block">{prod.desc[lang]}</p>
                </div>
              </div>
            </Link>
          </motion.div>
